...
async function sendSSEMessage(writer, data) {
  ...
  await writer.write(encoder.encode(`${SSE_DATA_PREFIX}${JSON.stringify(data)}${SSE_LINE_DELIMITER}`));
}
    ...
    const startStream = async () => {
      try {
        ...
        if (newMessage && newMessage.trim() !== "") {
          langChainMessages.push({ role: 'user', content: newMessage });
        }

        const eventStream = await submitQuestion(langChainMessages, interviewRoomId);
        ...
        for await (const event of eventStream) {
          if (event.event === "on_chat_model_stream") {
            ...
            if (token?.content) {
              const text = token.content;
              fullText += text;
              await sendSSEMessage(writer, { type: 'message', content: text });
            }
          } else if (event.event === "on_tool_start") {
            await sendSSEMessage(writer, {
              type: 'tool_start',
              tool: event.name || 'unknown_tool',
              input: event.data.input,
            });
          } else if (event.event === "on_tool_end") {
            ...
            await sendSSEMessage(writer, {
              type: 'tool_end',
              tool: toolMessage.lc_kwargs.name || 'unknown_tool',
              output: event.data.output,
            });
          }
        }

        await sendSSEMessage(writer, { type: 'done', content: fullText });

      } catch (error) {
        console.error('Error processing messages:', error);
        await sendSSEMessage(writer, { type: 'error', error: error.message || 'Failed to process messages.' });
      } finally {
        try {
          await writer.close();
        } catch (closeError) {
